...
export class RankingsClient {
     ...
     * @return OK
     */
    getRankingsForSeason(season: number): Promise<RankingDto[]> {
        let url_ = this.baseUrl + "/Rankings/{season}";
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processGetRankingsForSeason(_response);
        });
    }

    protected processGetRankingsForSeason(response: Response): Promise<RankingDto[]> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
...
export class SeasonsClient {
    ...
    getAllSeasons(): Promise<string[]> {
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processGetAllSeasons(_response);
        });
    }

    protected processGetAllSeasons(response: Response): Promise<string[]> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
